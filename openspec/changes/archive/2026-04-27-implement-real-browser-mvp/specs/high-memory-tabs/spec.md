...
The system SHALL target at least 8 GB of practical memory headroom per live browser tab on supported systems through browser configuration, process orchestration, and runtime policy management.

#### Scenario: Heavy workload remains active under target headroom
- **WHEN** a supported system runs a live tab whose workload memory demand grows within the configured high-memory target
...
- **WHEN** the host system cannot provide the configured per-tab memory target during live workload execution
...
The system SHALL monitor memory pressure and resource consumption for live browser tabs and related browser processes so that webox can apply preventative policies before catastrophic failure.

#### Scenario: Tab approaches critical memory threshold
- **WHEN** a live tab approaches a configured critical memory threshold
...
- **WHEN** live browser processes are active
...
The system SHALL apply mitigation policies when memory thresholds are exceeded during live browser execution, including warning, throttling, deprioritization, or recovery-oriented actions that reduce avoidable out-of-memory failures.

#### Scenario: Background activity is deprioritized under pressure
- **WHEN** a foreground live tab requires additional memory and the browser detects system memory pressure
...
- **WHEN** a live tab or browser subprocess terminates due to suspected memory exhaustion
