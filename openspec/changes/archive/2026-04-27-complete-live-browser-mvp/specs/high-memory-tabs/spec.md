...
- **WHEN** a supported system runs a tab whose workload memory demand grows within the configured high-memory target during live browser execution
...
- **WHEN** the host system cannot provide the configured per-tab memory target during live browser execution
...
The system SHALL monitor memory pressure and resource consumption for browser tabs and related browser processes so that webox can apply preventative policies before catastrophic failure using observed live browser execution data.

#### Scenario: Tab approaches critical memory threshold
- **WHEN** a tab approaches a configured critical memory threshold during live execution
...
- **THEN** webox continuously or periodically captures memory telemetry sufficient to attribute pressure to specific tabs or browser subsystems from live runtime execution
...
- **WHEN** a foreground tab requires additional memory and the browser detects system memory pressure during live execution
...
- **WHEN** a tab or browser subprocess terminates due to suspected memory exhaustion during live execution
